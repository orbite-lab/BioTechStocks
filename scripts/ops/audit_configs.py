    ...
    for a in cfg["assets"]:
        for ind in a["indications"]:
            ...
            if not m:
                ...
                continue
            # Skip SOM checks for generic-bucket indications (auto-generated by
            # expand_generic_mixes.py for big generics/biosimilar makers; their
            # salesM reflects franchise-share-of-disease without modeling
            # branded-style company_slice -- SOM=0 by design).
            is_generic_bucket = bool(m.get("_genericBucket", False))
            tam = tam_of(m); som = som_of(m)
            if not is_generic_bucket and tam > 0 and som > tam * 1.05:
                issues.append(("CRIT", f"{a['id']}.{ind['id']}: SOM ${som/1000:.1f}B > TAM ${tam/1000:.1f}B"))
            sales = m.get("salesM")
            if not is_generic_bucket and sales and som > 0 and som < sales * 0.9:
                issues.append(("CRIT", f"{a['id']}.{ind['id']}: SOM ${som/1000:.1f}B < reported sales ${sales/1000:.1f}B (model says peak already exceeded)"))
            if not is_generic_bucket and sales and som > sales * 5 and sales > 100:
            ...
            # Regions malformed
            if not is_generic_bucket and "regions" in m:
